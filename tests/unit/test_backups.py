...
def test_build_filename(tmp_path):
    ...
    backup_filename = build_filename(test_file, backup_folder, None, None)
    assert backup_filename.name == "test_file.txt"

    # Test with current_version and datetime_format
    backup_filename = build_filename(test_file, backup_folder, "1.0", "%Y%m%d_%H%M%S")
    ...
    backup_filename = build_filename(test_file, backup_folder, "1.0", None)
    assert backup_filename.name.startswith("v1.0_test_file.txt")

    # Test without current_version and with datetime_format
    backup_filename = build_filename(test_file, backup_folder, None, "%Y%m%d_%H%M%S")
